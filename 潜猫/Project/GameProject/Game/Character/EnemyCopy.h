...
private:
	...
	SearchNodeC* m_searchNode;		//探索中のノード
	CVector4D color;
...
public:
	...
	EnemyC(const CVector3D& pos, float emotion);
	//デストラクタ
	~EnemyC();
	/// <summary>
	/// 更新処理
	...
	//探索ノードを設定
	void SetSearchNode(SearchNodeC* node);
	//探索中のノードを取得
	SearchNodeC* GetSearchNode() const;
